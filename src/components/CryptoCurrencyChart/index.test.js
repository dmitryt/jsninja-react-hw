...
import { CryptoDataProvider } from '../../services/cryptoDataProvider';
...
jest.mock('../../services/cryptoDataProvider', () => {
	return {
		CryptoDataProvider: jest.fn().mockImplementation(() => {
			return {
				subscribe: ({ cb }) => {
					mockSubscribeCb = cb;
...
describe('Component CryptoCurrencyChart', () => {
	describe('Component Base', () => {
		beforeEach(() => {
			CryptoDataProvider.mockClear();
		});

		const setupComponent = (props, Component = WrappedComponent) => shallow(<Component dataProvider={new CryptoDataProvider()} {...props} />);
